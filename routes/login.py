...
from models.employees import Employee  # import modelu Employee
...
def login():
    ...
    # Sprawdź czy użytkownik to Restaurant
    ...
    # Sprawdź czy użytkownik to Employee
    ...
    if employee and check_password_hash(employee.password_hash, password):
        ...
        access_token = create_access_token(identity=employee.employee_id, additional_claims=additional_claims,
                                           expires_delta=timedelta(days=1))
        return jsonify(
            {'access_token': access_token, 'employee_id': employee.employee_id, 'message': 'Login successful'}), 200
